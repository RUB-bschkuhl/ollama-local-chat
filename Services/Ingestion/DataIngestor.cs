...
namespace OllamaRAGChat.Services.Ingestion;
    ...
    IngestionCacheDbContext ingestionCacheDb)
{
    ...
    {
        var vectorCollection = vectorStore.GetCollection<string, SemanticSearchRecord>("data-ollamaragchat-ingested");
        ...
        foreach (var modifiedDoc in modifiedDocs)
        {
            ...
            modifiedDoc.Records.AddRange(newRecords.Select(r => new IngestedRecord { Id = r.Key, DocumentId = modifiedDoc.Id + "-" + DateTime.UtcNow.Ticks }));
